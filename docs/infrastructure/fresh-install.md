...
Estado desta entrega: baseline **validada localmente em PostgreSQL descartável**, não aplicada ainda ao Supabase/Coolify real. Não há dump nem dados do ambiente perdido. O Supabase ativo foi inventariado no Coolify e está saudável, mas a API está parada por ausência total de variáveis de ambiente e o serviço Supabase não possui backup agendado. Nenhum segredo é versionado aqui.
