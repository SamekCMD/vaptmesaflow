...
Estado desta entrega: baseline **validada localmente e aplicada ao Supabase real em 20/09/2026**. O preflight encontrou zero tabelas Vapt antes do bootstrap. As migrations 01–07 foram executadas em transações individuais e registradas com SHA-256; a verificação posterior confirmou 7 registros, zero divergências de checksum, 18/18 tabelas esperadas com RLS, os 2 buckets e `public.orders` na publicação `supabase_realtime`. Não há dump nem dados do ambiente perdido. Em 21/09/2026, a API foi configurada no Coolify e o commit `25d7642` da branch de recuperação foi publicado com sucesso; `/health` e `/health/ready` retornaram HTTP 200. O frontend do commit `9976d6e` foi promovido na Vercel para produção com as variáveis públicas novas e carregou em `https://www.vapt.app.br` sem erros de console. O serviço Supabase ainda não possui backup agendado. Nenhum segredo é versionado aqui.

Checagem pública em 21/09/2026: `www.vapt.app.br` retornou HTTP 200; `vapt.app.br` retornou 308 para `https://www.vapt.app.br/`; a API retornou HTTP 200 em `/health` e `/health/ready`; e uma requisição com `Origin: https://www.vapt.app.br` recebeu `Access-Control-Allow-Origin` para essa origem. A landing foi carregada no Brave sem erros ou warnings de console. Esses sinais não substituem os testes autenticados de Auth, membership, Storage e Realtime, que continuam pendentes e pertencem ao smoke test funcional sem alterar o WIP Auth/Resend.
